...
func (d *Datasource) query(ctx context.Context, pCtx backend.PluginContext, query backend.DataQuery) (backend.DataResponse, error) {
	...
	datasourceSettings := pCtx.DataSourceInstanceSettings.JSONData
	var jsonData JSONData
	err = json.Unmarshal(datasourceSettings, &jsonData)
	if err != nil {
		return backend.DataResponse{}, fmt.Errorf("unmarshal: %w", err)
	}

	tables := jsonData.Tables
	contextPrompt := jsonData.Context
	engine := input.Engine
	...
	if err != nil {
		return backend.DataResponse{}, fmt.Errorf("marshal payload: %w", err)
	}
	...
	if !ok || (status != "OK" && status != "HELP") {
		return backend.DataResponse{}, fmt.Errorf("%w: invalid status: %s", errRemoteRequest, status)
	}
	span.AddEvent("Status OK")

	responseValue, _ := body["response"]
	responseStr, ok := responseValue.(string)
	if !ok {
		return backend.DataResponse{}, fmt.Errorf("%w: response is not string", errRemoteRequest)
	}
	span.AddEvent("SQL request retrieved")

	values := []string{responseStr}
	keys := []string{"sql"}
	...
	dataResp := backend.DataResponse{
		Frames: []*data.Frame{
			data.NewFrame(
				...
				data.NewField(valueKey, nil, values),
			),
		},
	}
	span.AddEvent("Frame created")
...
func (d *Datasource) CheckHealth(ctx context.Context, _ *backend.CheckHealthRequest) (*backend.CheckHealthResult, error) {
	...
	if err != nil {
		return newHealthCheckErrorf("request error on url : " + d.settings.URL), nil
	}
	defer func() {
